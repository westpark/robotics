...
class Controller(remote.Controller):
    ...
    def drive(self):
        self.robot.forward(stop_after_secs=1.0)
        self.robot.turn(direction="left", extent=0.5, speed=0.75, stop_after_secs=1.0)        
        self.robot.backward(stop_after_secs=1.0)
        self.robot.turn(direction="right", extent=0.5, speed=0.75, stop_after_secs=1.0)        
        self.robot.backward(stop_after_secs=1.0)
        sys.exit()
